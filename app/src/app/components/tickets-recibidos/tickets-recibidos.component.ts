...
export class TicketsRecibidosComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        private srvTrazaTicket: TsTrazaTrazaService,
        private svrEstadosTickets: TsEstadosTicketService,
        private messageService: MessageService,
        private confirmationService: ConfirmationService,
        private svrNotificaciones: NotificacionesService,
        private svrParametros: ParametrosService,
        private svrSolped: SolPedService,
        private srvTrazaSolped: TrazaSolpedService,
    ) {
    ...
    ngOnInit() {
        ...
        this.verBotonModificar =
            JSON.parse(localStorage.getItem('roles')).find((rol) => rol.codigo == this.rolEstados) != null ? 1 : 0;
    ...
    verTraza(ticket: TicketServicio) {
        ...
        this.svrTicket.getTrazasTicket(ticket.idTicketServicio).subscribe((data) => {
            ...
            if (ticket.idEstadoActual == 2) {
                ...
                this.ticketDetalle.justificacionEstadoActual =
                    'Generado Automaticamente por el sistema al leer el ticket';
                this.ticketDetalle.fechaEstadoActual = formatDate(
                    new Date().toString(),
                    'yyyy-MM-dd HH:mm:ss',
                    'en-US',
                );
        ...
        if (ticket.idEstadoActual == 2) {
            ...
            this.srvTrazaTicket.nuevoTraza(nueva).subscribe((data) => {
                this.svrTicket.actualizarTicket(ticket).subscribe(async (data) => {
                    ...
                    this.svrEstadosTickets
                        .obtenerEstadoSiguiente(ticket.idTicketServicio, mostrarEstado)
                        .subscribe((data) => {
                            ...
                            this.estados.forEach((dato) => {
                                this.estadosItems.push({ label: dato.nombre, value: dato.idEstadoTicket });
                            });
            ...
            this.svrEstadosTickets.obtenerEstadoSiguiente(ticket.idTicketServicio, mostrarEstado).subscribe((data) => {
                this.nuevaTraza.idTicketServicio = ticket.idTicketServicio;
                this.estados = data;
                this.estadosItems = null;
                this.estadosItems = [];
        ...
        for (let file of event.files) {
            ...
            if (existeimg.length > 0) {
                ...
                this.messageService.add({
                    key: 'tc2',
                    severity: 'error',
                    summary: 'Nombre del archivo YA INGRESADO al sistema',
            ...
            if (file.name.indexOf('#') != -1 || file.name.indexOf('%') != -1 || file.name.indexOf('/') != -1) {
                ...
                this.messageService.add({
                    key: 'tc2',
                    severity: 'error',
                    summary: 'Nombre del archivo con caracteres #, $, %, / NO PERMITIDOS',
                });
                archis.files.splice(archis.files.length - 1, 1);
                return false;
            }
            if (file.name.length > this.MAX_CARACTERES_NAME_ARCHI) {
                ...
                this.messageService.add({
                    key: 'tc2',
                    severity: 'error',
                    summary: 'Nombre del archivo Excede los 150 caracteres',
                });
                archis.files.splice(archis.files.length - 1, 1);
                return false;
            }
    ...
    async guardarNueva(archis2?: FileUpload) {
        if (
            this.nuevaTraza.justificacion == null ||
            this.nuevaTraza.justificacion == ' ' ||
            this.nuevaTraza.justificacion == ''
        ) {
            this.messageService.clear();
        ...
        this.srvTrazaTicket.nuevoTraza(this.nuevaTraza).subscribe((data) => {
            this.svrTicket.actualizarTicket(this.ticketDetalle).subscribe(async (data) => {
                ...
                if (
                    +this.ticketDetalle.idGerenciaDestino === +this.GERENCIA_COMPRAS &&
                    +this.ticketDetalle.idEstadoActual === 4
                ) {
                    ...
                    let solped: SolpedModelo = {
                        ...(await this.svrSolped.getDetalleSolPedTicket(this.ticketDetalle.idTicketServicio))[0],
                    };
                    console.log(solped);
                    if (!Configuration.isEmpty(solped) && +solped.idEstadoActual < 5) {
                        ...
                        trazaSolped.idEstadoSolped = orden == 5 ? 2 : 1;
                        trazaSolped.estadoActual = orden == 5 ? 'APROBADO' : 'EN TICKET';
                        await this.srvTrazaSolped.guardarTraza(trazaSolped);
                    }
                }
                this.svrNotificaciones
                    .nuevaNotificacionRecibe(
                        'Cambio de estado del ticket: ' + this.ticketDetalle.idTicketServicio,
                        this.idUSuario,
                        this.ticketDetalle.idSegUsuario,
                        this.ticketDetalle.idServiciosGerencias,
                    )
                    .subscribe((data) => {
                        this.cargarLista();
                    });
