...
export class FtProductoGeneralComponent implements OnInit {
    ...
    constructor(
        ...
        private svrRoles: RolesUsuariosService,
    ) {
    ...
    ngOnInit() {
        ...
        this.idAdmProducto = this.actRouter.snapshot.params.idAdmProducto;
        ...
        this.rolAprobacionProd =
            JSON.parse(localStorage.getItem('roles')).find((rol) => rol.codigo == this.nombreAprodProd) != null
                ? false
                : true;
        this.rolPostValidado =
            JSON.parse(localStorage.getItem('roles')).find((rol) => rol.codigo == this.nombrePostValidad) != null
                ? true
                : false;
        this.rolValidacionProd =
            JSON.parse(localStorage.getItem('roles')).find((rol) => rol.codigo == this.nombreValProd) != null
                ? false
                : true;
        ...
        if (this.producto.esservicio != true) {
            if (!this.producto.idAdmTipoMedida) {
                ...
                this.messageService.add({
                    key: 'tc',
                    severity: 'error',
                    summary: 'Debe seleccionar el tipo de medida de un producto',
                });
                return false;
            }
            if (!this.producto.idAdmUnidadMedida) {
                ...
                this.messageService.add({
                    key: 'tc',
                    severity: 'error',
                    summary: 'Debe seleccionar la unidad de medida de un producto',
                });
                return false;
            }
            if (!this.producto.idAdmMaterialProducto) {
                ...
                this.messageService.add({
                    key: 'tc',
                    severity: 'error',
                    summary: 'Debe seleccionar tipo de material de un producto',
                });
                return false;
            }
            if (!this.producto.idAdmColorProducto) {
                ...
                this.messageService.add({
                    key: 'tc',
                    severity: 'error',
                    summary: 'Debe seleccionar el color del producto',
                });
                return false;
            }
        }
    ...
    consultarProductoImagenes(idAdmProducto: number) {
        this.srvProductoImagenes
            .consultarPorId(idAdmProducto)
            .toPromise()
            .then((results) => {
                ...
                results.forEach((prodImg) => {
                    ...
                    let img = {
                        source: imgPath,
                        alt: prodImg.titulo,
                        title: prodImg.titulo,
                        id: prodImg.idAdmImgProducto,
                        thumbnail: imgPath,
                    };
        ...
        this.srvProducto
            ...
            .then((result) => {
                ...
                this.svrUsuarios.getUserById(this.producto.idUsuarioCreacion).subscribe((data) => {
                    ...
                    this.rolValidacionProd = !(JSON.parse(localStorage.getItem('roles')).find(
                        (rol) => rol.codigo == this.nombreValProd,
                    ) != null
                        ? true
                        : false && (gerenciaActual != gerenciaCreacion ? true : false));
            ...
            .then((result) => {
                this.srvGerencias.getTodos().then((results) => {
            ...
            .then((result) => {
                if (this.producto.idAdmProducto != null) {
                    this.svrRespoFuncio
                        .getTodosPorProducto(this.producto.idAdmProducto)
                        .then((data) => {
                            ...
                            if (this.responsables_funcio.length > 0) {
                                ...
                                this.gerencias.forEach((gerencia) => {
                                    gerencia.ordenCalcu =
                                        ...
                                        this.responsables_funcio.find((respo) => {
                                            return respo == gerencia.idConfigGerencia;
                                        })
                                            ? 1
                                            : 0;
                                });
                        ...
                        .then((result) => {
                            if (this.responsables_validacion.length > 0) {
                                this.gerencias_2.forEach((gerencia) => {
                                    ...
                                    gerencia.ordenCalcu =
                                        gerencia.idConfigGerencia ==
                                        this.responsables_validacion.find((respo) => {
                                            return respo == gerencia.idConfigGerencia;
                                        })
                                            ? 1
                                            : 0;
    ...
    consultarDatosAdicionales(idAdmProducto: number) {
        this.srvProducto
            .consultarDatosAdicionales(idAdmProducto)
            .toPromise()
            .then((results) => {
                this.producto.adicionales = results;
            })
            .catch((err) => {
                console.log(err);
            });
    ...
    async consultarDatosComplementarios(idAdmProducto: number) {
        //const datos : ComplementariaProducto[] =  [...await this.srvProducto.consultarComplementarias(idAdmProducto).toPromise()];

        this.srvProducto
            .consultarComplementarias(idAdmProducto)
            .toPromise()
            .then((results) => {
                this.producto.complementarias = results;
            })
            .catch((err) => {
                console.log(err);
            });
    ...
    consultarAplicabilidad(idAdmProducto: number) {
        this.srvProducto
            .consultarAplicabilidad(idAdmProducto)
            .toPromise()
            .then((results) => {
                this.producto.aplicabilidad = results;
            })
            .catch((err) => {
                console.log(err);
            });
    ...
    verificarRoles() {
        ...
        this.verSolComplementaria =
            this.rolesUsr.buscarRolPorCodigo(this.rolFtComplementarias).length > 0 ? false : true;
    ...
    cargaInicial() {
        ...
        this.srvUnidaMedidas
            .consultarTodos()
            .toPromise()
            .then((results) => {
                this.unidadMedidas = results;
            })
            .catch((err) => {
                console.log(err);
            });
        ...
        this.srvTiposDesagregacion
            .consultarTodos()
            .toPromise()
            .then((results) => {
                this.tiposDesagregacion = results;
            })
            .catch((err) => {
                console.log(err);
            });
        ...
        for (let file of event.files) {
            ...
            if (!reg.test(file.name)) {
                ...
                this.messageService.add({
                    key: 'tc',
                    severity: 'error',
                    summary: `Nombre del archivo NO PERMITIDO. Ejemplo permitido: lampara-3.jpg`,
                    life: 5000,
                });
            ...
            if (file.name.length > 120) {
                ...
                this.messageService.add({
                    key: 'tc',
                    severity: 'error',
                    summary: 'Nombre del archivo Excede los 120 caracteres',
                    life: 5000,
                });
                archis.files.splice(archis.files.length - 1, 1);
                return false;
    ...
    async nuevoProducto(imgs: FileUpload) {
        switch (this.tabIndex) {
            case 0:
                /* 	if (this.producto.existenciaMinima == null) {
						this.messageService.clear();
						this.messageService.add({ key: 'tc', severity: 'error', summary: 'Debe Ingresar la existencia minima' });
						return false;
					} */
                if (this.producto.nombre == null || this.producto.nombre.trim().length == 0) {
                    ...
                    this.messageService.add({
                        key: 'tc',
                        severity: 'error',
                        summary: 'Debe ingresar el nombre del producto o servicio',
                    });
                    return false;
                }
                ...
                if (this.producto.uso == null || this.producto.uso.length == 0) {
                    ...
                    this.messageService.add({
                        key: 'tc',
                        severity: 'error',
                        summary: 'Debe ingresar el uso del producto o servicio',
                    });
                    return false;
                }
                ...
                if (!this.imagenesTemp.files) {
                    ...
                    this.messageService.add({
                        key: 'tc',
                        severity: 'error',
                        summary: 'Debe ingresar al menos 1 imagen',
                    });
                    return false;
                }
                ...
                this.producto.idAdmGrupoProducto = !this.producto.idAdmGrupoProducto
                    ? -1
                    : this.producto.idAdmGrupoProducto;
                this.producto.idAdmSubGrupoProducto = !this.producto.idAdmSubGrupoProducto
                    ? -1
                    : this.producto.idAdmSubGrupoProducto;
            ...
            accept: () => {
                ...
                this.srvProductoImagenes.eliminar(imagenSelect.idAdmImgProducto).then(() => {
                    ...
                    this.messageService.add({
                        key: 'tc',
                        severity: 'success',
                        summary: 'Foto Eliminada con exito!',
                    });
                    this.productoImagenes = [];
                    this.consultarProductoImagenes(this.idAdmProducto);
                });
        ...
        this.svrConfirmacion.confirm({
            ...
            accept: () => {
                ...
                if (this.producto.aprobado == 1 && this.aprobacionAnterior == 0) {
                    ...
                    this.srvProducto.actualizar(this.producto, 4).then((data) => {
                        this.verInfUser = false;
                        this.newProducto = false;
                        this.consultarDatosProducto(this.idAdmProducto);
                        this.consultarProductoImagenes(this.idAdmProducto);
    ...
    validarInfoProducto() {
        this.svrConfirmacion.confirm({
            ...
            accept: () => {
                ...
                this.srvProducto.actualizar(this.producto, 5).then((data) => {
                    this.verInfUser = false;
                    this.newProducto = false;
                    this.consultarDatosProducto(this.idAdmProducto);
                    this.consultarProductoImagenes(this.idAdmProducto);
    ...
    edit() {
        ...
        this.srvProducto
                        ...
                        this.imagenesTemp.files.forEach((img) => {
                            ...
                            this.srvProductoImagenes.insertar(data).then((result) => {
                                ...
                                this.messageService.add({
                                    key: 'tc2',
                                    severity: 'info',
                                    summary: `Imagen: ${img.name} insertada correctamente`,
                                });
                            });
                        });
                    }
            ...
            .then((result) => {
                if (this.responsables_funcio.length > 0) {
                    this.svrRespoFuncio.delPorProducto(this.producto.idAdmProducto).then((data) => {
                        let respodata: ResponFuncionales = {};
            ...
            .then((result) => {
                if (this.responsables_validacion.length > 0) {
                    this.srvRespoValida.delPorProducto(this.producto.idAdmProducto).then((data) => {
            ...
            .then((result) => {
                ...
                this.svrRoles.getUsuariosPorRol(this.rolEnvioNotificacion).then((usuario) => {
                    usuario.forEach(async ({ idSegUsuario }) => {
                        let subNombre: string = this.producto.nombre.substring(0, 55);
                        await this.svrNotificaciones
                            .enviarNotificacionPorUsuario(
                                `Producto: ${this.producto.codigo}-${subNombre} fue modificado`,
                                this.idUsusarioSesion,
                                idSegUsuario,
                            )
                            .toPromise();
                    });
    ...
    onChangeGrupo(event) {
        ...
        this.srvGrupos
            .consultarSubGruposPorIdAdmGrupo(idAdmGrupoProducto)
            .toPromise()
            .then((results) => {
                this.subgrupos = results;
            })
    ...
    onChangeTipoMedida(event) {
        this.disabledUnidadMedidas = false;
        this.srvTiposMedida
            .consultarUnidadesMedidasPoridAdmTipoMedida(event.value)
            .toPromise()
            .then((results) => {
                this.unidadMedidas = results;
            })
