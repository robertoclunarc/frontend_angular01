...
export class NotificacionesComponent implements OnInit {
    @Input() verBread: number = 1;

    cols: any[];
    notificaciones: Notificacion[] = [];
    user: User;

	nroNoLeidasUser :number = 0;
    ...
    ngOnInit() {
        ...
        this.srvNotificaciones.getAllNotificacionesPorUsuario(this.user.idSegUsuario).subscribe((notificaciones) => {
            this.notificaciones = notificaciones;
        });

        this.intervalNotificaciones = interval(15000).subscribe((x) => {
            this.srvNotificaciones
                .getAllNotificacionesPorUsuario(this.user.idSegUsuario)
    ...
    leidas() {
        this.srvNotificaciones.getNotificacionesLeidas(this.user.idSegUsuario).subscribe((notificaciones) => {
            this.notificaciones = notificaciones;
    ...
    noLeidas() {
        this.srvNotificaciones.getNotificacionesNoLeidas(this.user.idSegUsuario).subscribe((notificaciones) => {
            this.notificaciones = notificaciones;
			this.nroNoLeidasUser = notificaciones.length;
        });
    }
    ...
    porFecha() {
        ...
        this.srvNotificaciones
            .getNotificacionesPorFecha(this.user.idSegUsuario, formattedDesde, formattedHasta)
            .subscribe((notificaciones) => {
                this.notificaciones = notificaciones;
                this.cerrarDialogo();
            });
