...
export class SolpedsdetalleComponent implements OnInit {
    ...
    // @Output() sinEstadoChange: EventEmitter<number> = new EventEmitter<number>(); //para el two way biding
    ...
    ngOnInit() {
        ...
        this.svrDetalleSolped
            .getDetalleDetSolpedTodos(this.idSolped, this.sinEstado)
            .toPromise()
            .then((data) => {
                data.map((det) => {
                    det.opcion_aprobar = 1;
                    return det;
                });
